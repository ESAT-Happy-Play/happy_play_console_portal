...
import DepositFilterModal from '../../../components/modals/accountingModals/DepositFilterModal';
...
const WithdrawalsTable = ({ data }) => {
    ...
    return (
        <div style={{ paddingLeft: 20, paddingRight: 20 }}>
            ...
            <CustomTable
                headers={["Reference Number", "Display Name", "Amount", "Status", "Payment Method", "Date"]}
                pagination={
                    <StyledPagination
                    ...
                    displayList.slice(page * rowsPerPage, page *
                        rowsPerPage + rowsPerPage).map((row, i) => (
                            <StyledTableRow key={i} sx={{ background: `${row.status == 3 ? COLORS.transparentRed : null} !important` }} onClick={() => handleEdit(row)}>
                                ...
                                <StyledTableCell align="center" sx={{ color: row.status == 2 ? COLORS.yellow : row.status == 3 ? COLORS.redWarn : null }}>{mockStatus.find((e) => e.id == row.status).name}</StyledTableCell>
                                <StyledTableCell align="center" >{mockPaymentMethod.find((e) => e.id == row.paymentMethod).name}</StyledTableCell>
            ...
            {openSuccess &&
                ...
                >
                    ...
                    <DialogContent>
                        <DialogContentText>Successfully updated withdrawal!</DialogContentText>
                    </DialogContent>
