...
const MasterAgentSendCredit = ({ isOpenAdd, handleCloseAdd, handleCallback, balance }) => {
  ...
  const handleDownlineData = async () => {
    setPageLoader(true);
    let url = `${process.env.REACT_APP_API_URL}/users/downline`;

    let response = await GETFetch(url);
    setPageLoader(false);

    if(response.status) {
      setdonwline(response.data.donwline);
    }
              ...
              <div className="divContent">
                ...
                <div className="right">
                  <TextField 
                    ...
                    label="Select Agent" sx={{ width: "100%" }} defaultValue="" variant="outlined" size="small" select>
...
export default MasterAgentSendCredit
