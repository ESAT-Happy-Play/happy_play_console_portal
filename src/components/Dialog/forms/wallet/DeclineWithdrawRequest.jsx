...
const DeclineWithdrawRequest = ({ isOpenAdd, handleCloseAdd, handleCallback, objData }) => {
  ...
  const handleDeclineCreditOkay = async () => {
    ...
    let response = await PATCHFetch(`${process.env.REACT_APP_API_URL}/credits/withdraw/requests/${requestId}?isapprove=0&declinereason=${formData.declinereason}`, {});
  ...
  return (
    <>
      ...
      <MessageDialog
        ...
        title={ "Confirmation" } 
        content={ "Are you sure you want to decline withdrawal request?" }
        color={ "error" }
