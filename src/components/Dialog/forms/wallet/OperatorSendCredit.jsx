...
const OperatorSendCredit = ({ isOpenAdd, handleCloseAdd, handleCallback, balance }) => {
  ...
  const handleDownlineData = async () => {
    setPageLoader(true);
    let url = `${process.env.REACT_APP_API_URL}/masteragents?rowsperpage=100&pagenumber=1&companyid=${loginObj.companyId}`;

    let response = await GETFetch(url);
    setPageLoader(false);

    if(response.status) {
      setdonwline(response.data.masterAgents);
    }
              ...
              <div className="divContent">
                ...
                <div className="right">
                  <TextField 
                    ...
                    label="Select Master Agent" sx={{ width: "100%" }} defaultValue="" variant="outlined" size="small" select>
                    <MenuItem value=''><em>Select Master Agent</em></MenuItem>
                    { 
...
export default OperatorSendCredit
