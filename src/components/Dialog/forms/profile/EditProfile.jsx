...
const EditProfile = ({ isOpenEdit, handleCloseEdit, handleCallback, objData }) => {

  const [pageLoader, setPageLoader] = useState(false);
  const [pofileId, setpofileId] = useState('');
  ...
  useEffect(() => {
    if (objData !== null) {
      setpofileId(objData.pofileId);
      setpofileName(objData.profileCategory);
    }
  }, [objData]);
  ...
  const handleProfileOkay = async () => {
    ...
    let response = await PATCHFetch(`${process.env.REACT_APP_API_URL}/users/accessprofile?featureid=${pofileId}`, objformdata);
    if(response.status) {
              ...
              (objData !== null) ?
                <form onSubmit={ handleSubmit(finalStepHandler) } noValidate>
                  <div className="div-bottom">
                    ...
                    <TextField defaultValue={pofileName} variant="outlined" size="small" />
                  </div>
                  ...
                  <div className="div-content-wrapper">
                    <div className="div-content">
                      ...
                      <div style={{marginTop:'15px'}}>
                        <FormGroup>
                          <FormControlLabel control={<Checkbox defaultChecked={(objData.g1_superadmin === 1) ? true : false } {...register('g1_superadmin', {})} />} label="Super Admin" />
